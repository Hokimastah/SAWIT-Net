...
class SAWITTrainer:
    ...
    def fit_two_stage(self, base_source, inc_source, mode: str = "full", report: bool = False) -> Dict[str, object]:
        ...
        base_before = evaluate(self.model, base_loader_before, self.device, report=report)

        inc_loader = self.fit_incremental(inc_source, mode=mode)
        ...
        base_after = evaluate(self.model, base_loader_after, self.device, report=report)
        inc_after = evaluate(self.model, inc_loader_after, self.device, report=report)
        all_after = evaluate(self.model, all_loader, self.device, report=report)
